...
class TicketService:
    ...
    # --- EJERCICIO 2: Método watchers ---
    def watchers(self, ticket_id: int) -> list[User]:
        """
        Devuelve la lista de observadores del ticket: el solicitante y,
        si existe, el técnico asignado, sin repetir usuarios.
        """
        ...
        if ticket.assignee_id:
            ...
            if technician.id not in seen:
                seen.add(technician.id)
                result.append(technician)
                
        return result

    # --- EJERCICIO 3: Asignación con control de DuplicateAssignmentError ---
    def assign(
        self,
        ticket_id: int,
        technician_id: int,
        ...
        # Validar si ya está asignado al mismo técnico antes de modificar historial o notificar
        if ticket.assignee_id == technician_id:
            raise DuplicateAssignmentError("El ticket ya se encuentra asignado a este técnico.")
